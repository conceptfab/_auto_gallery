import { validateFilePath, validateFileName, validateFolderPathDetailed } from '../../src/utils/pathValidation';

function test(name: string, result: boolean, expected: boolean) {
  if (result === expected) {
    console.log(`✅ ${name}`);
  } else {
    console.error(`❌ ${name} - FAILED (Got: ${result}, Expected: ${expected})`);
    process.exit(1);
  }
}

function testValue(name: string, result: any, expected: any) {
    if (result === expected) {
      console.log(`✅ ${name}`);
    } else {
      console.error(`❌ ${name} - FAILED (Got: '${result}', Expected: '${expected}')`);
...
const pathWithSlashes = 'foo//bar///baz';
// Expect: 'foo/bar/baz'
