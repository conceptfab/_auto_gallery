 ...
 */
export function convertToSignedUrl(directUrl: string, baseGalleryUrl: string): string {
...
export function generateListUrl(folder: string = ''): string {
  const FILE_LIST_URL = process.env.FILE_LIST_URL || 'https://conceptfab.com/file-list.php';
  const expires = Math.floor(Date.now() / 1000) + TOKEN_EXPIRY_SECONDS;
...
const FILE_UPLOAD_URL = process.env.FILE_UPLOAD_URL || 'https://conceptfab.com/file-upload.php';
const FILE_DELETE_URL = process.env.FILE_DELETE_URL || 'https://conceptfab.com/file-delete.php';
const FILE_RENAME_URL = process.env.FILE_RENAME_URL || 'https://conceptfab.com/file-rename.php';
const FILE_MKDIR_URL = process.env.FILE_MKDIR_URL || 'https://conceptfab.com/file-mkdir.php';
const FILE_MOVE_URL = process.env.FILE_MOVE_URL || 'https://conceptfab.com/file-move.php';

/**
 * Generuje token dla uploadu plików
 */
export function generateUploadToken(folder: string = ''): { token: string; expires: number; url: string } {
  const expires = Math.floor(Date.now() / 1000) + TOKEN_EXPIRY_SECONDS;
  const token = crypto
    .createHmac('sha256', SECRET_KEY)
 ...
 */
export function generateDeleteToken(path: string): { token: string; expires: number; url: string } {
  const expires = Math.floor(Date.now() / 1000) + TOKEN_EXPIRY_SECONDS;
  const token = crypto
    .createHmac('sha256', SECRET_KEY)
 ...
 */
export function generateRenameToken(oldPath: string, newName: string): { token: string; expires: number; url: string } {
  const expires = Math.floor(Date.now() / 1000) + TOKEN_EXPIRY_SECONDS;
  const token = crypto
    .createHmac('sha256', SECRET_KEY)
 ...
 */
export function generateMkdirToken(parentFolder: string, folderName: string): { token: string; expires: number; url: string } {
  const expires = Math.floor(Date.now() / 1000) + TOKEN_EXPIRY_SECONDS;
  const token = crypto
    .createHmac('sha256', SECRET_KEY)
 ...
 */
export function generateMoveToken(sourcePath: string, targetFolder: string): { token: string; expires: number; url: string } {
  const expires = Math.floor(Date.now() / 1000) + TOKEN_EXPIRY_SECONDS;
  const token = crypto
    .createHmac('sha256', SECRET_KEY)
